...
class ChatListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SafeArea(child: Scaffold(body: ChatListViewBody()));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: ChatList(
        ...
        searchConfig: SearchConfig(
          ...
          onSearch: (value) async {
            ...
            final list = chatListController.chatListMap.values
                .where(
                  (chat) => chat.name.toLowerCase().contains(value.toLowerCase()),
                )
                .toList();
            return list;
          },
        ...
        header: ChatFilterChipsSection(chatListController: chatListController),
        menuConfig: ChatMenuConfig(
          ...
          muteStatusCallback: (result) => chatListController.updateChat(
            ...
            (previousChat) => previousChat.copyWith(
              settings: previousChat.settings.copyWith(muteStatus: result.status),
            ),
          ),
          pinStatusCallback: (result) => chatListController.updateChat(
            result.chat.id,
            (previousChat) => previousChat.copyWith(
              settings: previousChat.settings.copyWith(pinStatus: result.status),
            ),
          ),
        ),
        tileConfig: ListTileConfig(
          ...
          timeConfig: LastMessageTimeConfig(
            timeBuilder: (time){
              ...
              return Text(DateFormat('h:mm a').format(time), style: AppTextStyle.interMedium10.copyWith(color: Theme.of(context).colorScheme.onSurface),);
            }
          ),
          ...
          userAvatarConfig: UserAvatarConfig(
            avatarBuilder: (chat) {
              return CircleAvatar(
                ...
                child: SvgPicture.asset(//TODO: replace with user image
          ...
          onTap: (chat) {
            Navigator.of(context, rootNavigator: true).pushNamed(AppChatView.name);
          },
          padding: const EdgeInsets.all(12),
          middleWidgetPadding: const EdgeInsets.symmetric(horizontal: 12),
          lastMessageTextStyle: AppTextStyle.interMedium14.copyWith(color: Theme.of(context).colorScheme.onSurface),
